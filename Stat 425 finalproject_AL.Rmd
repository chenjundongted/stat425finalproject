...
```{r}
full_1 = lm(Price~.,data = Real_Estate)
summary(full_1)
```
...
```{r}
full_2 = lm(Price~Date+Age+Distance+Stores+Latitude+(1/(Longitude))+Trans_Month, data=Real_Estate)
drop1(full_2,test = 'F')
```
...
```{r}
model_1 = lm(Price~Date+Age+Distance+Stores+Latitude+(1/(Longitude)), data=Real_Estate)
summary(model_1)
...
model_2 = lm(Price ~ Date + Age + Distance + Stores + Latitude + (1/(Longitude)),data = normalized_real)
summary(model_2)
```
